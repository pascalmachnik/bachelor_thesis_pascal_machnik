...
mass.setRange("fit", 5300, 5940)  # Set the fit range for the mass observable

# Define the signal model
# The signal is modeled as two double sided Crystal Ball functions
signal_mean = r.RooRealVar("signal_mean", "signal_mean", 5620, 5610, 5630)
signal_sigma = r.RooRealVar("signal_sigma", "signal_sigma", 10, 1, 30)
...
signal_yield = r.RooRealVar("signal_yield", "Signal yield", 100, 0, 1e6)
combinatorial_yield = r.RooRealVar("comb_yield", "Combinatorial yield", 100, 0, 1e6)
...
# Create a RooDataSet
df.Snapshot("DecayTree", f"/ceph/users/pmachnik/temp/r3_{args.track_type}_tempRM_data.root", branches)
f = r.TFile(f"/ceph/users/pmachnik/temp/r3_{args.track_type}_tempRM_data.root")
...
if os.path.exists(f"/ceph/users/pmachnik/temp/r3_{args.track_type}_tempRM_data.root"):
    os.remove(f"/ceph/users/pmachnik/temp/r3_{args.track_type}_tempRM_data.root")
