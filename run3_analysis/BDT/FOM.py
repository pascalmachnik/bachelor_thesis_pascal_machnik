...
df_MC = r.RDataFrame("DecayTree", args.input_MC, ["Lambdab_DTF_Lambda_PV_M", "BDTG"])  # MC signal
df_data = r.RDataFrame("DecayTree", args.input_data, ["Lambdab_DTF_Lambda_PV_M", "BDTG"])  # data

# Create snapshot of the dataframes
df_MC.Snapshot("DecayTree", f"/ceph/users/pmachnik/temp/r3_{args.track_type}_MC_reduced.root", ["Lambdab_DTF_Lambda_PV_M", "BDTG"])
df_data.Snapshot("DecayTree", f"/ceph/users/pmachnik/temp/r3_{args.track_type}_Data_reduced.root", ["Lambdab_DTF_Lambda_PV_M", "BDTG"])

# Load datasets 
treeDataAll = r.TChain("DecayTree") #data
treeDataAllMC = r.TChain("DecayTree") #MC

treeDataAll.Add(f"/ceph/users/pmachnik/temp/r3_{args.track_type}_Data_reduced.root") #data
treeDataAllMC.Add(f"/ceph/users/pmachnik/temp/r3_{args.track_type}_MC_reduced.root") #MC
...
while (BDTval<0.9995):
    ...
    f_new = r.TFile(f"/ceph/users/pmachnik/temp/r3_{args.track_type}_Data_rare.root","recreate")
    ...
    dfsiga_sel.Snapshot("treesig", f"/ceph/users/pmachnik/temp/r3_{args.track_type}_preselMC_rare_L.root", {"Lambdab_DTF_Lambda_PV_M"})

    f_news = r.TFile(f"/ceph/users/pmachnik/temp/r3_{args.track_type}_preselMC_rare_L.root")
    treesig = f_news.Get("treesig")
    nmc = treesig.GetEntries()
    print("Number MC events=", nmc)

    #fit
    Lb_M = r.RooRealVar("Lambdab_DTF_Lambda_PV_M", "Lambdab_DTF_Lambda_PV_M", 5620, 5300, 5940)
    ...
    mean_Lb = r.RooRealVar("mean_Lb", "mu", 5620, 5600, 5640)
    sigma_Lb = r.RooRealVar("sigma_Lb","sigma_Lb",6.,3.5,30.)
    ...
    can.SaveAs('../plots/FOM/R3/{}/Data_rare_dCB_Fit_{}BDT.pdf'.format(args.track_type, BDTval))
    can.Close()
...
hists.SetMarkerStyle(8)
hists.Draw("Phist")
c1.SaveAs(f"../plots/FOM/R3/TMVA_BDT_rare_Punzi_{args.track_type}.pdf")
c2.SaveAs(f"../plots/FOM/R3/TMVA_BDT_rare_{args.track_type}.pdf")
...
import os
if os.path.exists(f"/ceph/users/pmachnik/temp/r3_{args.track_type}_preselMC_rare_L.root"):
    os.remove(f"/ceph/users/pmachnik/temp/r3_{args.track_type}_preselMC_rare_L.root")
if os.path.exists(f"/ceph/users/pmachnik/temp/r3_{args.track_type}_Data_rare.root"):
    os.remove(f"/ceph/users/pmachnik/temp/r3_{args.track_type}_Data_rare.root")
if os.path.exists(f"/ceph/users/pmachnik/temp/r3_{args.track_type}_MC_reduced.root"):
    os.remove(f"/ceph/users/pmachnik/temp/r3_{args.track_type}_MC_reduced.root")
if os.path.exists(f"/ceph/users/pmachnik/temp/r3_{args.track_type}_Data_reduced.root"):
    os.remove(f"/ceph/users/pmachnik/temp/r3_{args.track_type}_Data_reduced.root")